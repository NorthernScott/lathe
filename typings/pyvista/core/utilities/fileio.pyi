...
def set_vtkwriter_mode(vtk_writer, use_binary=...):
...
def read(filename, force_ext=..., file_format=..., progress_bar=...):
...
def read_texture(filename, progress_bar=...):
...
def read_exodus(filename, animate_mode_shapes=..., apply_displacements=..., displacement_magnitude=..., read_point_data=..., read_cell_data=..., enabled_sidesets=...):
...
def read_meshio(filename, file_format=...):
...
def save_meshio(filename, mesh, file_format=..., **kwargs):
